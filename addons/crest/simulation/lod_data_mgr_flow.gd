class_name CrestLodDataMgrFlow
extends CrestLodDataMgr
## Flow "sim": not a simulation - cleared each frame and re-filled from the
## registered flow inputs. Port of Crest's LodDataMgrFlow.

var settings := CrestSimSettingsFlow.new()

var _clear: CrestRDCompute
var _inject: CrestRDCompute


func init_mgr(p_resolution: int, p_layers: int, p_settings: CrestSimSettingsFlow) -> void:
	settings = p_settings if p_settings else CrestSimSettingsFlow.new()
	init_sim(p_resolution, p_layers, RenderingDevice.DATA_FORMAT_R16G16_SFLOAT, false)
	_clear = CrestRDCompute.from_file(rd, "res://addons/crest/shaders/sim/clear_rg16f.glsl")
	_inject = CrestRDCompute.from_file(rd, "res://addons/crest/shaders/sim/inject_flow.glsl")


## inputs: array of Dictionaries:
##   mode: 0 = flow map (additive), 1 = fixed direction (replace)
##   rect_center/rect_half_size: Vector2 world XZ
##   fixed_velocity: Vector2 (mode 1)
##   strength: float (mode 0)
##   texture: Texture2D or null (mode 0 flow map)
func update_sim(lod_transform: CrestLodTransform, cascade_current: RID, inputs: Array) -> void:
	...
	_clear.dispatch(resolution / CrestConstants.THREAD_GROUP_SIZE, resolution / CrestConstants.THREAD_GROUP_SIZE, layer_count,
		{0: set_clear}, CrestRDCompute.pack_push_constants(PackedFloat32Array([float(resolution), 0.0, 0.0])))
...
func _dispatch_inject(lod_transform: CrestLodTransform, cascade_current: RID, input: Dictionary) -> void:
	...
	var vel: Vector2 = input.get("fixed_velocity", Vector2.ZERO)
	...
	var pc := PackedFloat32Array([
		...
		1.0 if input.get("texture") != null else 0.0,
	])
...
## Bridges a Godot Texture2D to an RD texture RID for compute sampling.
...
func _get_fallback_tex() -> RID:
	if not _fallback_tex.is_valid():
		...
		fmt.height = 1
		fmt.usage_bits = RenderingDevice.TEXTURE_USAGE_SAMPLING_BIT | RenderingDevice.TEXTURE_USAGE_CAN_UPDATE_BIT
		var data := PackedFloat32Array([0.5, 0.5, 0.0, 0.0]).to_byte_array()
