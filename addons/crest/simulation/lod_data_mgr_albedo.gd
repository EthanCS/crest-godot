class_name CrestLodDataMgrAlbedo
extends CrestLodDataMgr
## Albedo inputs: overrides the water scattering colour in registered areas.
## Port of Crest's LodDataMgrAlbedo.

var settings := CrestSimSettingsAlbedo.new()

var _clear: CrestRDCompute
var _inject: CrestRDCompute


func init_mgr(p_resolution: int, p_layers: int, p_settings: CrestSimSettingsAlbedo) -> void:
	settings = p_settings if p_settings else CrestSimSettingsAlbedo.new()
	init_sim(p_resolution, p_layers, RenderingDevice.DATA_FORMAT_R8G8B8A8_UNORM, false)
	_clear = CrestRDCompute.from_file(rd, "res://addons/crest/shaders/sim/clear_rgba8.glsl")
	_inject = CrestRDCompute.from_file(rd, "res://addons/crest/shaders/sim/inject_albedo.glsl")
...
func _dispatch_inject(lod_transform: CrestLodTransform, cascade_current: RID, input: Dictionary) -> void:
	...
	var tint: Color = input.get("tint", Color.WHITE)
